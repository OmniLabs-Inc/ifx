<template>
                          ...
                          <th>
                            <form @submit.prevent="getBinary">
                              <input type="text" class="text-search-input form-control p-2"
                                placeholder="Search Level Income" v-model="matching_income">
                            </form>
                          </th>
                      ...
                      <tbody v-if="!loading">
                        ...
                        <template v-else>
                          <tr class="activation-card-data" v-for="(data, index) in filteredItems" :key="index">
                            ...
                            <td>${{ parseFloat(data.level_income) }}</td>
  ...
  export default {
    ...
    computed: {
      filteredItems: function () {
        ...
        return this.tree.filter(function (item) {
          return item.level_income.toLowerCase().indexOf(searchQuery) !== -1
        });
      }
    },
    setup() {
    ...
    methods: {
      async getBinary() {
        this.loading = true;
        let response = await ApiClass.getRequest(`income/level-bonus?page=${this.page}&per_page=${this.perPageData}&level_income=${this.matching_income}&status=${this.status}`, true);
