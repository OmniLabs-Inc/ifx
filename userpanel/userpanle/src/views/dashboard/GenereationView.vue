...
export default {
    ...
    data() {
        return {
            richMediaData: {
                ...
                user_unique_id: null,
                status:0,
                avatar: 'https://app.ifxglobalpro.com/images/dashboard/profile/user-img2.png',
                children: [
    ...
    methods: {
        ...
        async getList(use_) {
            ...
            let response = await ApiClass.getRequest(`tree_node_generation?sponser_id=${find_id}`, true);
            if (response.data.status_code == "1") {
        ...
        async addList(sponser_id) {
            let response = await ApiClass.getRequest(`tree_node_generation?sponser_id=${sponser_id}`, true)
