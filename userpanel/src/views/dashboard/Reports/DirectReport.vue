                                  ...
                                  <table class="table">
                                      ...
                                      <tbody v-if="!loading">
                                          <tr class="activation-card-data" v-for="(data, index) in reportdata"
                                              :key="index">
                                              <!--<td>{{ index + 1 }}</td>-->
                                              <td> ${{ parseFloat(data.total_amount) }} </td>
                                              <td> {{ data.user_unique_id }}</td>
                                              <!--<td>{{ sponser_id }}</td>-->
                                              <td>{{ data.name }}</td>
                                              <!--<td>{{ data.email }}</td>-->
                                              <!--<td>{{ this.formatDateString(data.created_at) }}</td>-->
                                              <td>
...
//   },
  mounted() {
      ...
      if(localStorage.getItem('user')){
        let pp = JSON.parse(localStorage.getItem('user'));
        this.sponser_id = pp.user_unique_id;
  ...
  methods: {
      async getReport({ page , perpage}) {
            this.page = page;
          this.loading = true;
          let res = await ApiClass.getRequest(`direct/report?page=${page}&per_page=${perpage}&unique_id=${this.search}`, true);
          this.loading = false;
      ...
      formatDateString(dateString){
            ...
            // Month is zero-indexed, so we add 1 to get the correct month
