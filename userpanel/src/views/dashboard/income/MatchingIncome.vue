                  ...
                  <table class="table">
                    ...
                    <thead>
                      <tr class="activation-card-head">
                        ...
                        <th>
                          <input type="date" class="text-search-input form-control p-2" v-model="receive_date"
                            @change="getBinary">
                        </th>
                        <th>
                          <form @submit.prevent="getBinary">
                            <input type="text" class="text-search-input form-control p-2"
                              placeholder="Search Matching Income" v-model="matching_income">
                          </form>
                        </th>
                    ...
                    <tbody v-if="!loading">
                      ...
                      <template v-else>
                        <tr class="activation-card-data" v-for="(data, index) in filteredItems" :key="index">
                          ...
                          <td>{{ data.date }}</td>
                          <td>${{ parseFloat(data.matching_income) }}</td>

                          <td>
                            <span class="px-3 py-2 rounded-0"
                              :style="data.status == 'completed' ? 'background-color: rgba(85, 189, 108, 0.1254901961);color:#66c37b' : 'background-color:rgba(244, 67, 54, 0.1254901961);color:#f6685e'">
                              {{ data.status }}
                            </span>
                          </td>
                        </tr>
...
export default {
  ...
  computed: {
    filteredItems: function () {
      ...
      return this.tree.filter(function (item) {
        return item.matching_income.toLowerCase().indexOf(searchQuery) !== -1
      });
    }
  },
  setup() {
  ...
  methods: {
    async getBinary() {
      this.loading = true;
      let response = await ApiClass.getRequest(`income/matching?page=${this.page}&per_page=${this.perPageData}&receive_date=${this.receive_date}&matching_income=${this.matching_income}&status=${this.status}`, true);
