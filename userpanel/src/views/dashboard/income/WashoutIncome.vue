                    ...
                    <thead>
                      <tr class="activation-card-head">
                        ...
                        <th>
                          <form @submit.prevent="getBinary">
                            <input type="text" class="text-search-input form-control p-2"
                              placeholder="Search Washout Income" v-model="washout_income">
                          </form>
                        </th>
                    ...
                    <tbody v-if="!loading">
                      ...
                      <template v-else>
                        <tr class="activation-card-data" v-for="(data, index) in filteredItems" :key="index">
                          ...
                          <td>${{ parseFloat(data.washout_income) }}</td>
...
export default {
  ...
  methods: {
    async getBinary() {
      this.loading = true;
      let response = await ApiClass.getRequest(`income/wash?page=${this.page}&per_page=${this.perPageData}&date=${this.date}&matching_income=${this.matching_income}&washout_income=${this.washout_income}&status=${this.status}`, true);
